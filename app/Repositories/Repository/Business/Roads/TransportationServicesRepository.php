...
/**
 * Clase TransportationServicesRepository
 * @package App\Repositories\Repository\Business\Roads
 */
class TransportationServicesRepository extends Repository
{
    /**
     * Constructor de TransportationServicesRepository.
     *
     * @param App $app
    ...
    function model()
    {
        return TransportationServices::class;
    }
    ...
    /**
     * Obtener de la BD todas las necesidades de conservación de la via.
     *
     * @param string $code
     *
     * @return mixed
    ...
    /**
     * Obtener de la BD una necesidad de conservación por gid.
     *
     * @param $gid
     *
     * @return mixed
     ...
     *
     * @param array $data
     * @param TransportationServices $entity
     *
     * @return TransportationServices|null
     */
    public function updateFromArray(array $data, TransportationServices $entity)
    {
        if ($data['imagen']) {
            if (Storage::disk('inventory_roads')->exists($entity->imagen)) {
                Storage::disk('inventory_roads')->delete($entity->imagen);
            }
            $fileName = trans('transportation_services.image_path') . '_' . $entity->codigo . '_' . microtime() . '_' . $entity->gid . '.' . $data['imagen']->getClientOriginalExtension();
     ...
     * @param $image
     */
    public function addImage(TransportationServices $entity, $image)
    {
        $fileName = trans('transportation_services.image_path') . '_' . $entity->codigo . '_' . $entity->gid . '.' . $image->getClientOriginalExtension();
