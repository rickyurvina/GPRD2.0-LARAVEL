...
class BudgetItemController extends Controller
{
    ...
    {
        try {
            $response['view'] = view('business.planning.current_expenditure.budget_items.show',
    ...
    public function create(int $activityId, string $activityType = BudgetItem::ACTIVITY_TYPE_PROJECT)
    {
        try {
            ...
            if ($activityType === BudgetItem::ACTIVITY_TYPE_OPERATIONAL) {
                if ($data['incomes'] > 0) {
                    $response['modal'] = view('business.planning.projects.activities.budget_items.create', $data)->render();
                } else {
                    $response = [
                        'message' => [
                            'type' => 'warning',
                            'text' => trans('budget_item.messages.errors.no_incomes')
                        ]
                    ];
                }
            } else {
    ...
    public function edit(int $budgetItemId, string $activityType = BudgetItem::ACTIVITY_TYPE_PROJECT)
    {
        try {
            ...
            $response['modal'] = view('business.planning.projects.activities.budget_items.update', $data)->render();
            return response()->json($response);
        } catch (Throwable $e) {
            return response()->json(defaultCatchHandler($e));
        }
    ...
    {
        try {
            return $this->budgetItemProcess->dataShow($activityId);
        } catch (Throwable $e) {
            return datatableEmptyResponse($e, $e);
        }
    }
