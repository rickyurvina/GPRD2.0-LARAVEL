<div class="x_panel shadow" id="activity-admin-list">
    <div class="x_title">
        <h2 class="text-primary"><i class="fa fa-sliders"></i> {{ trans('staff_meetings.labels.admin_activities') }}
            (<span data-bind="text: countCompleted()"></span>/<span data-bind="text: activities().length"></span>)
        </h2>
                        ...
                        <div>
                            ...
                            <div class="check-list-item-title">
                                <input type="text" data-bind="textInput: newActivity, event: { keyup: addNewActivity }" class="text-border-less"
                                       placeholder="Agregar una actividad adicional/administrativa"
                                       maxlength="120" value="">
                            </div>
                        </div>
                    </li>
            ...
            <!-- ko foreach: adminActivities() -->
                <li class="check-list-item">
                    <div style="align-items: start">
                        ...
                        <!-- ko if: is_extra() -->
                        <div class="mr-4">
                            <span style="font-size: smaller" class="badge badge-success">NUEVA</span>
                        </div>
                        <!-- /ko -->
                        <div>
    ...
    $(() => {
        ...
        var viewModel = function () {
            ...
            self.addNewActivity = (type, event) => {
                if (event && event.which === 13 && self.newActivity() && self.newActivity().trim() !== '') {
                    var item = new Activity();
                    item.name = self.newActivity().trim().replaceAll('"', '');
                    item.type('{{ \App\Models\Business\StaffActivity::TYPE_ADMIN }}');
            ...
            self.adminActivities = ko.computed(() => {
                return self.activities().filter(item => item.type() === '{{ \App\Models\Business\StaffActivity::TYPE_ADMIN }}');
            });

            self.countCompleted = ko.computed(() => {
                return self.adminActivities().filter(item => item.is_completed()).length;
            });
            ...
            self.editActivity = (data) => {
                ...
                url = url.replace('__ID__', data.id)
                pushRequest(url, '#edit-activities', null, 'get', {'type': '{{ \App\Models\Business\StaffActivity::TYPE_ADMIN }}'}, false);
        ...
        ko.applyBindings(vm, document.getElementById('activity-admin-list'));
    });
</script>
