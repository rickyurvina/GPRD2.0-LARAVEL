<div id="activity-coordination-list">
                    ...
                    <div>
                        ...
                        <div class="check-list-item-title">
                            <input type="text" data-bind="textInput: newActivity, event: { keyup: addNewActivity }" class="text-border-less"
                                   placeholder="Agregar una actividad de coordinación entre áreas"
                                   maxlength="120" value="">
                        </div>
                    </div>
                </li>
...
<script>
    $(() => {
        ...
        var viewModel = function () {
            ...
            self.addNewActivity = (type, event) => {
                if (event && event.which === 13 && self.newActivity().trim() !== '') {
                    ...
                    item.type('{{ \App\Models\Business\StaffActivity::TYPE_COORDINATION }}');
                    ...
                    $.ajax('{{ route('store.activity.staff') }}', {
                        ...
                        self.newActivity('');
                        $('#coordination_count').html(vm.activities().length);
        ...
        let activities = JSON.parse('{!! str_replace("\u0022", "\\\\\"", json_encode($activity->coordinations, JSON_HEX_APOS | JSON_HEX_QUOT)) !!}'
        ...
        ko.applyBindings(vm, document.getElementById('activity-coordination-list'));
    });
</script>
