@permission('index.project_indicators.tracking')
...
<div class="row mb-15">
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            ...
            @permission('data.index.project_indicators.tracking')
            <div class="x_content">
...
<script>
    $(() => {
        let dataTable = build_datatable($('#projects_tb'), {
            ajax: '{!! route('data.index.project_indicators.tracking') !!}',
            columns: [
                ...
                {data: 'end_date', width: '10%', class: 'text-center'},
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (row.indicators.length === 0) {
                            $('#indicators' + row.id + '-data' + row.id, data).attr('disabled', 'disabled');
                            $('#indicators' + row.id + '-data' + row.id, data).attr('data-original-title', '{{ trans('indicator_tracking.labels.no_indicators') }}');
                        }
