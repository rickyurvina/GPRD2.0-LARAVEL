@permission('edit.prioritization.plans_management')
<div>
    <div class="page-title">
        ...
        <div class="title_right hidden-xs">
            <ol class="breadcrumb pull-right">
                ...
                <li class="active">{{ trans('prioritization.labels.edit') }}</li>
            </ol>
        </div>
    </div>
                                                                ...
                                                                <select class="form-control select2" data-scope="{{ $scope->id }}" data-weight="{{ $scope->weight }}"
                                                                    ...
                                                                    @foreach($criteria->answers as $answer)
                                                                        <option value="{{ $answer->value }}"
                                                                                @if(isset($prioritization->configuration()[$scope->id][$criteria->id]) && $answer->value == $prioritization->configuration()[$scope->id][$criteria->id])
                                                                                    selected
                                                                            @endif >
                                                                            {{ $answer->name }}
                                                                        </option>
    ...
    <footer class="navbar-default navbar-fixed-bottom text-center">
        ...
        <div>
            ...
            <button id="btnDeletePrioritization" name="btnDeletePrioritization" class="btn btn-danger" type="button">
...
<script>
    $(() => {
        ...
        let deleteButton = $('#btnDeletePrioritization')
        let selectInputs = $('.select2')
        let prioritizationIndex = $('#index_value')
        const configuration = {!!  $prioritization->configuration !!}
        ...
        // Initialize scope total preview elements
        let initializePreviewElements = () => {
            $('[id^=scope_]').each((index, element) => {

                let scopeId = $(element).attr('id').replace('scope_', '')
                let scorePreview = $('#score_' + scopeId);
                let scopeSum = 0
                let scopeWeight = 0

                $('[data-scope^=' + scopeId + ']').each((index, criteria) => {
                    scopeWeight = parseFloat($(criteria).attr('data-weight')) || 0
                    scopeSum += parseInt($(criteria).val()) || 0
                })

                $(element).text(scopeSum)
                scorePreview.text((scopeSum * scopeWeight).toFixed(1))
            })
        ...
        /**
         * Verificar si la priorización ha sido modificada
         *
         * @returns {boolean}
         */
        let prioritizationChanged = () => {

            let equalConfigurations = equalArrays(configuration, createPrioritizationObject())

            equalConfigurations ? prioritizeButton.prop('disabled', true) : prioritizeButton.prop('disabled', false)
        ...
        prioritizeButton.on('click', () => {
            ...
            confirmModal(confirmMessage, () => {
                ...
                let url = '{{ route('update.edit.prioritization.plans_management', ['id' => $prioritization->id]) }}'
        ...
        deleteButton.on('click', () => {
            ...
            let confirmMessage = '{{ trans('prioritization.messages.confirm.delete') }}'

            confirmModal(confirmMessage, () => {

                let url = '{{ route('destroy.prioritization.plans_management', ['id' => $prioritization->id]) }}'

                pushRequest(url, null, null, 'DELETE', {
                    _token: '{{ csrf_token() }}'
                });
            });
        })
