...
<div class="modal-content">
    ...
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel"><i class="fa fa-money"></i> {{ trans('components.title') }}
        </h4>
    </div>
    <div class="x_content">
        <div class="" role="tabpanel" data-example-id="togglable-tabs">
            ...
            <form role="form"
                  action="{{ route($url, ['indicatorId' => $indicatorId]) }}"
                                ...
                                <div class="x_panel">
                                    ...
                                    <div class="x_content">
                                        ...
                                        <div class="item form-group">
                                            ...
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                                <select class="disabledInputs form-control select2 select2_measuring_unit"
                                                        id="measuring_unit" name="measure_unit_id">
                                                    @foreach($measuringUnits as $value)
                                                        <option value="{{ $value->id }}"
                                                                data-abbv="{{ $value->abbreviation }}"
                                        ...
                                        <div id="dynamic_files">
                                            @include('business.planning.projects.logic_frame.components.indicators.partial.inputs')
                                        </div>
    ...
    $(() => {
        ...
        let createChart = (index, value) => {
            ...
            myChart.setOption({
                ...
                yAxis: {name: $("#measuring_unit option:selected").val()}
            });
        };
        ...
        let createChartToleranceMin = (index, min) => {
            ...
            myChart.setOption(toleranceOptions);
            myChart.setOption({
                title: {
                    text: '{{ $entity->name }}',
                    subtext: '{{ trans('plan_indicators.labels.base_line') }}: ' + '{{ $entity->base_line }}' + '\n{{ trans('plan_indicators.labels.base_line_year') }}: ' + '{{ $entity->base_line_year }}'
        ...
        $("#measuring_unit", $planIndicatorUpdateFm).on('change', (element) => {
        ...
        $("#base_line", $planIndicatorUpdateFm).focusout(() => {
            let baseLineValue = $("#base_line").val();
            let measurementFrequencyPerYear = $("#measurement_frequency_per_year").val();
            $("#base_lineLabel").text(baseLineValue);
            $("#lineBaseValue").text(baseLineValue);
            if (measurementFrequencyPerYear > 0) {
                count = measurementFrequencyPerYear;
            }

            baseline = Number(baseLineValue);
        ...
        let printTableTolerance = () => {
            ...
            @endforeach
                ...
                toleranceOptions.series[0].data = min;
            toleranceOptions.series[1].data = max;
