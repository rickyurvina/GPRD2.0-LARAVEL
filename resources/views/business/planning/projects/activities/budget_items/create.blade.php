@if(currentUser()->can('create.items.activities.projects.plans_management') || currentUser()->can('create.items.operational_activities.current_expenditure_elements.budget.plans_management'))
...
<div class="modal-content" id="myModal">
    ...
    @if(isset($activityType) && $activityType === $BudgetItem::ACTIVITY_TYPE_OPERATIONAL)
        <form method="post"
              action="{{ route('store.create.items.operational_activities.current_expenditure_elements.budget.plans_management', ['activityId' => $activity->id, 'activityType' => $activityType]) }}"
              class="form-horizontal form-label-left" id="budget_item_create_fm">
    @else
        <form method="post" action="{{ route('store.create.items.activities.projects.plans_management', ['activityId' => $activity->id]) }}" class="form-horizontal form-label-left"
