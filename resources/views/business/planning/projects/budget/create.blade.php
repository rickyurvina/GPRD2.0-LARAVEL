...
<div class="modal-content" id="myModal">
    ...
    <form method="post" action="{{ route('store.create.index.budget.projects.plans_management') }}"
        ...
        <div class="modal-body">
            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_content">
                        ...
                        <div class="col-md-12">
                            <table class="table table-bordered detail-table">
                                ...
                                <tr>
                                    ...
                                    <td>@isset($projectFiscalYear->project->subprogram) {{ $projectFiscalYear->project->subprogram->parent->code }} @endisset</td>
                                    <td>@isset($projectFiscalYear->project->subprogram) {{ $projectFiscalYear->project->subprogram->parent->description }} @endisset</td>
                                </tr>

                                <tr>
                                    <td>{{ trans('budget_item.labels.sub_program') }}</td>
                                    <td>@isset($projectFiscalYear->project->subProgram) {{ $projectFiscalYear->project->subProgram->code }} @endisset</td>
                                    <td>@isset($projectFiscalYear->project->subProgram) {{ $projectFiscalYear->project->subProgram->description }} @endisset</td>
                                </tr>

                                <tr>
                                    <td>{{ trans('budget_item.labels.project') }}</td>
                                    <td>@isset($projectFiscalYear->project) {{ $projectFiscalYear->project->cup }} @endisset</td>
                                    <td>@isset($projectFiscalYear->project) {{ $projectFiscalYear->project->name}} @endisset</td>
                                </tr>

                                <tr>
                                    <td>{{ trans('budget_item.labels.executingUnit') }}</td>
                                    <td>@isset($projectFiscalYear->project->executingUnit) {{ $projectFiscalYear->project->executingUnit->code }} @endisset</td>
                                    <td>@isset($projectFiscalYear->project->executingUnit) {{ $projectFiscalYear->project->executingUnit->name }} @endisset</td>
                                </tr>
                    ...
                    <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="activity_id">
                            {{ trans('budget_item.labels.activity') }} <span class="text-danger">*</span>
                        </label>
                        <div class="col-md-9 col-xs-12">
                            <select class="form-control select2" id="activity_id" name="activity_id" required>
                                <option value=""></option>
                                @foreach($projectFiscalYear->activitiesProjectFiscalYear as $act)
                                    <option value="{{ $act->id }}" data-code="{{ $act->code }}" data-area-code="{{ $act->area->code }}">
                                        {{ $act->code }} - {{ $act->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="item form-group">
                        ...
                        <div class="col-md-9">
                            <select class="form-control select2" name="institution_id" id="institution_id" data-width="100%" required>
                                <option value=""></option>
                ...
                <div class="form-group group_amount">
                    ...
                    <div class="col-md-9 col-xs-12">
                        <div class="input-group">
                            ...
                            <input type="text" name="amount" id="amount" class="form-control mt-0" min="{{ $BudgetItem::MIN_ALLOWED_VALUE }}" maxlength="16"
                                   max="{{ $BudgetItem::MAX_ALLOWED_VALUE }}">
                        </div>
                    </div>
                </div>
...
<script>
    $(() => {
        ...
        budget_item_key = [
            '{{ $BudgetItem::CODE_00 }}', // Área
            @if($projectFiscalYear->project->subprogram)'{{ $projectFiscalYear->project->subprogram->parent->code }}', @else '{{ $BudgetItem::CODE_00 }}', @endif // Prog
            @if($projectFiscalYear->project->subprogram)'{{ $projectFiscalYear->project->subprogram->code }}', @else '{{ $BudgetItem::CODE_00 }}', @endif // Subp
            @if($projectFiscalYear->project)'{{ $projectFiscalYear->project->cup }}', @else '{{ $BudgetItem::CODE_00 }}', @endif // Proy
            @if($projectFiscalYear->project->executingUnit)'{{ $projectFiscalYear->project->executingUnit->code }}', @else '{{ $BudgetItem::CODE_00 }}', @endif // UE
            '{{ $BudgetItem::CODE_000 }}', // Act
            '{{ $BudgetItem::CODE_0 }}.{{ $BudgetItem::CODE_00 }}.{{ $BudgetItem::CODE_00 }}.{{ $BudgetItem::CODE_00 }}', // Nat.Grup.Subg.Ítem
            '{{ $BudgetItem::FUN }}', // Competencia
            '{{ $BudgetItem::CODE_00 }}.{{ $BudgetItem::CODE_00 }}.{{ $BudgetItem::CODE_00 }}.{{ $BudgetItem::CODE_00 }}', // OG.DG.CAT.BUB
        ...
        $('.select2').select2({
            ...
            switch ($(e.currentTarget).attr('id')) {
                case 'activity_id':
                    budget_item_key[5] = $(e.currentTarget).find(':selected').data('code');
                    budget_item_key[0] = $(e.currentTarget).find(':selected').data('area-code');
                    break;
        ...
        let budget_tb = $('#budget_tb').DataTable();
