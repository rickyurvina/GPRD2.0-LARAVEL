...
<div id="myModal" class="modal-content">
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel"><i class="fa fa-road"></i> {{ trans('sewer.labels.new_type') }}
        </h4>
    </div>

    <div class="mt-5">
        <form role="form" action="{{ route('store.create.sewer_type.inventory_roads_catalogs') }}" method="post"
              class="form-horizontal form-label-left" id="sewer_type_create_fm" novalidate>

            @csrf

            <div class="item form-group">
                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="codigo">
                    {{ trans('sewer.labels.code') }} <span class="text-danger">*</span>
                </label>
            ...
            <div class="item form-group">
                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="descrip">
                    {{ trans('sewer.labels.description') }} <span class="text-danger">*</span>
                </label>
                <div class="col-md-6 col-sm-6 col-xs-12">
...
<script>
    $(() => {
            ...
            descrip: {
                ...
                remote: {
                    ...
                    data: {
                        ...
                        model: 'App\\Models\\Business\\Roads\\Catalogs\\SewerType',
                    }
                }
            }
        };
        ...
        $validateDefaults.messages = {
            descrip: {
                remote: '{{ trans('sewer.messages.validations.sewer_type_uniqueDesc') }}'
            }
        };

        $form.validate($.extend(false, $validateDefaults));

        let datatable = $('#sewer_type_tb').DataTable();
