@permission('edit.mines.inventory_roads')
...
<div class="modal-content" id="intersection_edit">
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">
            <i class="fa fa-automobile"></i> {{ trans('mines.labels.edit') }}
        </h4>
    </div>
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_content">
            <form role="form" action="{{ route('update.edit.mines.inventory_roads', ['gid' => $entity->gid]) }}"
                  method="post"
                  class="form-horizontal form-label-left" id="mines_edit_fm">
                @csrf
                ...
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <select class="form-control select2"
                            ...
                            @foreach($typeMines as $typeMine)
                                <option value="{{ $typeMine->descrip }}"
                                        @if($entity->tipo == $typeMine->descrip) selected @endif>
                                    {{ $typeMine->descrip }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <select class="form-control select2"
                                id="fuente" name="fuente" required>
                            <option value="">{{ trans('app.labels.select') }}</option>
                            @foreach($sources as $source)
                                <option value="{{ $source->descrip }}"
                                        @if($entity->fuente == $source->descrip) selected @endif>
                                    {{ $source->descrip }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <select class="form-control select2"
                            ...
                            <option value="">{{ trans('app.labels.select') }}</option>
                            @foreach($materialMines as $materialMine)
                                <option value="{{ $materialMine->descrip }}"
                                        @if($entity->material == $materialMine->descrip) selected @endif>
                                    {{ $materialMine->descrip }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <input type="text" name="lat" id="lat"
                               class="form-control col-md-7 col-sm-7 col-xs-12"
                               placeholder="{{ trans('mines.placeholders.lat') }}" value="{{ $entity->lat }}"/>
                    </div>
                </div>
                ...
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <textarea name="observ" id="observ" class="form-control col-md-7 col-sm-7 col-xs-12" rows="5"
                                  placeholder="{{ trans('mines.placeholders.observ') }}">{{ $entity->observ }}</textarea>
                    </div>
                </div>
