<script>
    $(() => {
        ...
        El.toCamelCase = function (str) {
            var cc = str.replace(/-([a-z])/g, function (g) {
        ...
        function El(ref) {
            ...
            if (utils.isString(ref)) {
                if (ref[0] == '<') {
                    ...
                    node = Browser.getElement(ref) || document.createElement(ref);
                } else {
        ...
        // These functions could be called when validating i/o options;
        cli.isFile =
            cli.isDirectory = function (name) {
                return false;
            };
        ...
        function ImportFileProxy(gui) {
            ...
            function find(src) {
                ...
                var retn = datasets.reduce(function (memo, d) {
                    ...
                    lyr = utils.find(d.layers, function (lyr) {
                        return lyr.name == src;
                    });
        ...
        internal.initProjLibrary = function (opts, done) {
            ...
            // skip loaded libs
            libs = libs.filter(function (name) {
                return !mproj.internal.mproj_search_libcache(name);
            });
        ...
        function drawOutlineLayerToCanvas(obj, canv, ext) {
            ...
            var darkStyle = {strokeWidth: style.strokeWidth, strokeColor: (style.strokeColors) ? style.strokeColors[1] : '#334'},
                lightStyle = {
                    strokeWidth: style.strokeWidth,
                    strokeColor: (style.strokeColors) ? style.strokeColors[0] : '#86c927'
                };
            var filter;
            ...
            function getOverlayStyle(lyr, o) {
                ...
                if (topId > -1) {
                    ...
                    if (isPinned) {
                        ...
                        style = selectionHoverStyles[type];
                    } else {
                        style = hoverStyles[type];
                    }
                    ids.push(topId);
        ...
        internal.getCanvasDisplayStyle = function (lyr) {
            var styleIndex = {
                ...
                fields = internal.getCanvasStyleFields(lyr).filter(function (f) {
        ...
        function SymbolDragging2(gui, ext, hit) {
            ...
            function updateSymbol2(node, d, id) {
                var o = internal.svg.importStyledLabel(d); //
        ...
        function ClickText(ref) {
            ...
            var _max = Infinity,
                _min = -Infinity,
                _formatter = function (v) {
                    return String(v);
                },
                _validator = function (v) {
                    return !isNaN(v);
                },
                _parser = function (s) {
                    return parseFloat(s);
        ...
        function ImportControl(gui, opts) {
            ...
            ImportControl.receiveFiles = receiveFiles;
            ...
            function readFile(file) {
                ...
                if (arrayShapesContent.length > 0) {
                    reader.addEventListener('loadend', function (e) {
                        arrayShapesContent.forEach(function (element) {
                            if (name === element[0]) {
                                importFileContent(name, element[1]);
                            }
                        });
                    });
                } else {
            ...
            function importNewDataset(fileType, fileName, content, importOpts) {
                ...
                if (size > 4e7) {
                    //gui.showProgressMessage('CARGANDO ARCHIVOS');
                    delay = 35;
                }
                setTimeout(function () {
                    var dataset;
        ...
        function RepairControl(gui) {
            ...
            function showIntersections(XX, arcs) {
                ...
                if (n > 0) {
                    ...
                    readout.html(utils.format('<span class="icon"></span>%s line intersection%s <img class="close-btn">', n, utils.pluralSuffix(n)));
        ...
        function LayerControl(gui) {
            ...
            function turnOn() {
                isOpen = true;
                el.findChild('div.info-box-scrolled').css('max-height', '300px');
                render();
                el.show();
            }
            ...
            function renderLayer(lyr, dataset, opts) {
                ...
                html += rowHTML('Nombre:', '<span class="layer-name colored-text dot-underline">' + getDisplayName(lyr.name) + '</span>', 'row1');
                html += '<img class="close-btn" style="display: none" draggable="false" >';
                if (opts.pinnable) {
                    html += '<img class="pin-btn unpinned" draggable="false" src="{{ mix("vendor/shapes/images/eye.png") }}">';
                    html += '<img class="pin-btn pinned" draggable="false" src="{{ mix("vendor/shapes/images/eye2.png") }}">';
                }
                html += '</div>';
                return html;
            }
            ...
            function onKeyDown(evt) {
                ...
                if (kc == 27) {
                    ...
                    if (kc == 13) { // enter
                        ...
                        // Assumes that cursor is at end of text
        ...
        var arrayShapeNames = $.parseJSON('{!! $shapes !!}'.replace(/\n/g, "\\\\n").replace(/\r/g, "\\\\r").replace(/\t/g, "\\\\t"));
        var arrayShapeNamesDefault = @json($shapesDefault);
        var arrayShapes = [];
        var arrayShapesContent = [];
        ...
        function startEditing() {
            ...
            if (arrayShapeNames.length && arrayShapeNamesDefault.length) {

                arrayShapeNames.forEach(function (element) {
                    var file = new File([''], element);
                    arrayShapes.push(file);
                });
                var count_shapes = 0;
                arrayShapeNames.forEach(function (element) {
                    ...
                    xhr.addEventListener("loadend", function () {
                        if (xhr.status === 200) {
                            fileReader.onload = function (evt) {
                                var result = [element, evt.target.result];
                                arrayShapesContent.push(result);
                                count_shapes++;
                                if (arrayShapeNames.length == count_shapes) {
                                    addFiles();
                                }
                            };
            ...
            } else if (arrayShapeNamesDefault.length) {

                arrayShapeNamesDefault.forEach(function (element) {
                    var file = new File([''], element);
                    arrayShapes.push(file);
                });
                var count_shapes = 0;
                    ...
                    xhr.addEventListener("loadend", function () {
                        if (xhr.status === 200) {
                            fileReader.onload = function (evt) {
                                ...
                                arrayShapesContent.push(result);
                                count_shapes++;
                                if (arrayShapeNamesDefault.length == count_shapes) {
                                    ImportControl.receiveFiles(arrayShapes);
                                    document.getElementById('import_shapes').click();
                                }
                            };
                            fileReader.readAsArrayBuffer(xhr.response);
                        }
            ...
            } else if (arrayShapeNames.length) {
                ...
                xhr.addEventListener("loadend", function () {
                    if (xhr.status === 200) {
                        fileReader.onload = function (evt) {
                            var result = [element, evt.target.result];
                            arrayShapesContent.push(result);
                            count_shapes_default++;
                            if (arrayShapeNamesDefault.length == count_shapes_default) {
                                ImportControl.receiveFiles(arrayShapes);
                                document.getElementById('import_shapes').click();
                            }
                        };
