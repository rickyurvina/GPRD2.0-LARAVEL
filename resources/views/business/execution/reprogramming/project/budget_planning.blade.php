    ...
    $(() => {
            ...
            if (args.item.parent != null) {
                ...
                $.each(dataView.getItems(), (index, item) => {
                    if (item.parent === parent.id) {
                        amount += parseFloat(item[grid.getColumns()[args.cell].field]) ? parseFloat(item[grid.getColumns()[args.cell].field]) : 0;
                    }
                });
        ...
        const updateRowTotal = (row) => {
            ...
            for (let i = currentMonth; i < grid.getColumns().length; i++) {
                rowTotal += parseFloat(row[grid.getColumns()[i].field]) || 0;
            }
            row.total_month = rowTotal;
            ...
            if (row.parent != null) {
                ...
                for (let i = currentMonth; i < grid.getColumns().length; i++) {
                    rowTotal += parseFloat(parent[grid.getColumns()[i].field]) || 0;
                }
                parent.total_month = rowTotal;
                dataView.updateItem(parent.id, parent);
            }
        };
        ...
        $('#btn-save').on('click', () => {
            ...
            if (!valid) {
                ...
                pushRequest('{{ route('update.budgetary.project.reprogramming.reforms_reprogramming.execution', ['projectId' => $projectFiscalYear->id]) }}', null, null, 'POST', {
